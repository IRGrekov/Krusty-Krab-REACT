...
export const Order = () => {
  const ingredients = useAppSelector(
    (store) => store.burgerIngredients.burgerIngredients
  )
  const orders = useAppSelector((store) => store.webSocket.orders)
      ...
      order && (
        <div className={styles.order_info}>
          ...
          <ul className={styles.scroll}>
            {orderIngredientsForImage!.map((item) => (
              <li className={styles.item} key={item._id}>
                <img
                  ...
                  alt={item.name}
                />
                <p className={`${styles.text} text_type_main-default`}>
                  {item.name}
                </p>
                <p className={`${styles.price} text text_type_digits-default`}>
                  {orderIngredients?.filter((i) => i._id === item._id).length} x{' '}
                  {item.price}
